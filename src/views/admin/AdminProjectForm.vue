<template>
  <div class="max-w-4xl mx-auto animate-fade-in">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <GlassCard variant="dark" :animated="true" title="Imagem do Projeto">
        <div class="space-y-4">
          <div 
            ...
            :class="imagePreview ? 'border-purple-500/50' : 'border-white/20 hover:border-white/40'"
          >
            <input 
              ref="fileInput"
              type="file"
            ...
            <div v-if="!imagePreview" class="p-12 text-center">
              ...
              <p class="text-white font-medium mb-2">Clique para fazer upload</p>
          ...
          <p v-if="errors.image" class="text-red-400 text-sm">{{ errors.image }}</p>
        </div>
      </GlassCard>

      <!-- Basic Info -->
      <GlassCard variant="dark" :animated="true" :delay="100" title="Informações Básicas">
        <div class="space-y-4">
          <div>
          ...
          <div>
            ...
            <textarea 
              ...
              :class="errors.description ? 'border-red-500/50' : 'border-white/10'"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                list="categories"
                class="w-full px-4 py-3 bg-black/30 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all text-white placeholder-gray-500"
                :class="errors.category ? 'border-red-500/50' : 'border-white/10'"
                placeholder="Ex: E-commerce, Landing Page"
              />
              <datalist id="categories">
                <option v-for="cat in projectsStore.categories" :key="cat" :value="cat" />
              </datalist>
            ...
            <div>
              ...
              <input 
                ...
                type="url"
                class="w-full px-4 py-3 bg-black/30 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all text-white placeholder-gray-500"
                :class="errors.project_link ? 'border-red-500/50' : 'border-white/10'"
                placeholder="https://..."
              />
              <p v-if="errors.project_link" class="text-red-400 text-sm mt-1">{{ errors.project_link }}</p>
            </div>
          </div>
        </div>
      </GlassCard>
      ...
      <!-- Settings -->
      <GlassCard variant="dark" :animated="true" :delay="200" title="Configurações">
        <div class="flex items-center justify-between">
          <div>
            <h4 class="font-medium text-white">Projeto Ativo</h4>
            <p class="text-gray-400 text-sm">Projetos inativos não aparecem no portfólio</p>
          </div>
          <button 
            type="button"
      ...
      <div v-if="projectsStore.error" class="p-4 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-center">
        {{ projectsStore.error }}
      </div>
    </form>
  </div>
