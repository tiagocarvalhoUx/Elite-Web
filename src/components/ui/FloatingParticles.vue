<template>
  ...
  >
    ...
    <canvas ref="canvasRef" class="particles-canvas" :style="canvasStyle" />
...
const animate = () => {
  ...
  particles.forEach((particle, i) => {
    ...
    context.globalAlpha =
      particle.alpha * (0.7 + Math.sin(particle.pulsePhase) * 0.3);
    context.fill();
    ...
    if (props.connections) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < props.connectionDistance) {
          ...
          context.strokeStyle = `rgba(167, 139, 250, ${opacity})`;
